...
export async function fitbitConnected(): Promise<boolean> {
  ...
  try {
    return await withCache("fitbit:connected", TTL.connected, async () =>
...
export async function fitbitHeartRateForDay(
  ...
  try {
    return await withCache(
      ...
      TTL.hr,
      async () => (await fitbitProvider()).getHeartRateForDay(date, withIntraday),
    );
  } catch {
    return null;
  }
...
export async function fitbitSleep(date: string): Promise<SleepSummary | null> {
  if (!(await fitbitConnected())) return null;
  try {
    return await withCache(`fitbit:sleep:${date}`, TTL.sleep, async () =>
      (await fitbitProvider()).getSleep(date),
    );
  } catch {
    return null;
  }
...
export async function fitbitDailyActivity(
  ...
  if (!(await fitbitConnected())) return null;
  try {
    return await withCache(`fitbit:daily:${date}`, TTL.daily, async () =>
...
export async function fitbitWeight(
  ...
  try {
    return await withCache(
      ...
      TTL.weight,
      async () => (await fitbitProvider()).getWeightLog(fromDate, toDate),
    );
  } catch {
    return [];
  }
